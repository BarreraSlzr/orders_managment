...
const calculateDateRange = (timeZone: string, date?: string) => {
  if (date) {
    const start = sql`timezone(${timeZone}, ${date}::timestamp)::date`;
    const end = sql`timezone(${timeZone}, ${date}::timestamp) + interval '1 day' - interval '1 second'`;
    return { start, end };
  }

  // Default to today
  const todayStart = sql`timezone(${timeZone}, now())::date`;
  const todayEnd = sql`timezone(${timeZone}, now() + interval '1 day')::date - interval '1 second'`;
