...
export default function OnboardingsPageClient({
  ...
  const staff = !isSystemAdmin
    ...
    : [];
  const canEditManagers = role === "admin" && canManageMultipleManagers;
  ...
  const canCreateManager =
    role === "admin" && isSystemAdmin && canManageMultipleManagers;
                ...
                {!canCreateManager && (
                  <p className="mt-2 text-xs text-slate-400">
                    {canManageMultipleManagers
                      ? "System admins only."
                      : "Feature multi-manager no disponible para este tenant."}
                  </p>
                )}
              </div>
            </div>
