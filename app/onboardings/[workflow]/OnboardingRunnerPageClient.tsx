...
export default function OnboardingRunnerPageClient({
  canManageMultipleManagers = true,
}: {
  canManageMultipleManagers?: boolean;
}) {
  ...
  const availableByRole = getAvailableWorkflows(effectiveRole).some(
  ...
  const blocksManagerWorkflow =
    definition?.id === "onboard-manager" &&
    !canManageMultipleManagers &&
    !isEditMode;
  const available = availableByRole && !blocksManagerWorkflow;
  ...
  if (!available) {
    return (
      <div className="rounded-2xl border bg-white/80 p-6 text-sm text-slate-600">
        {blocksManagerWorkflow
          ? "Multi-manager feature is not enabled for this tenant."
